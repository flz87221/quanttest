...
def filter_stocks(context, bar_dict):
    ...
    last_year = datetime.datetime(year = current_date.year - 1, month = current_date.month, day = current_date.day).strftime('%Y-%m-%d')
    
    growth_lastyear_df = get_fundamentals(
        query(
            #营业收入同比增长率
            fundamentals.financial_indicator.inc_operating_revenue,
            #扣除非经常损益净利润同比增长率
    ...
    two_yearsago = datetime.datetime(year = current_date.year - 2, month = current_date.month, day = current_date.day).strftime('%Y-%m-%d')
    
    growth_2yearsago_df = get_fundamentals(
        query(
            #营业收入同比增长率
            fundamentals.financial_indicator.inc_operating_revenue,
            #扣除非经常损益净利润同比增长率
    ...
    growth_stocks = set(fundamental_df.columns.values) & set(growth_lastyear_df.columns.values) & set(growth_2yearsago_df.columns.values)
    context.stocks = []
